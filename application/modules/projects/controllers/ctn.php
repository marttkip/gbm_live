...
{ 
	...
	*/
	public function index($project_id) 
	{
		...
		$ctn_where = 'is_ctn = 1 AND project_id ='.$project_id;
		...
		if($ctn_query->num_rows() > 0)
		{
			...
			$community_group_where = 'community_group.community_group_status = 1 AND community_group.community_group_id = seedling_production.nursery_id AND community_group.community_group_id NOT IN (SELECT nursery_id FROM orders WHERE project_id = '.$project_id.')';
			...
			$order_where = 'is_ctn = 0 AND community_group.project_id ='.$project_id;
			$order_table = 'community_group';
			$order_by = 'community_group.community_group_id';

			//pagination
			$segment = 4;
			$this->load->library('pagination');
			$config['base_url'] = site_url().'tree-planting/ctn-detail/'.$project_id;
			$config['total_rows'] = $this->users_model->count_items($order_table, $order_where);
			...
			$orders_query = $this->ctn_model->get_all_project_nurseries($order_table, $order_where, $config["per_page"], $page);
			// $order_limit = 5;
	...
	*/
	public function add_ctn($project_id) 
	{
		...
		{
			if($this->ctn_model->add_ctn($project_id))
			{
		...
		redirect('tree-planting/ctn-detail/'.$project_id);
	}
	...
	public function print_ctn_recievable($order_id)
	{
		...
		//var_dump($data['community_group_info']); die();
